    ...
    <animated.div style={navSpring}>
      ...
      >
        <Container maxWidth="lg">
          <Toolbar disableGutters sx={{ py: 0.5 }}>
            {/* Logo */}
            <Box
              sx={{ display: 'flex', alignItems: 'center', gap: 1, cursor: 'pointer', flexGrow: { xs: 1, md: 0 } }}
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', px: 2, pb: 1 }}>
          <IconButton onClick={() => setDrawerOpen(false)} sx={{ color: '#FAF7F4' }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <Box sx={{ px: 2, mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
          <HomeRepairServiceIcon sx={{ color: '#C4956A', fontSize: 24 }} />
          <Typography sx={{ color: '#FAF7F4', fontWeight: 900, letterSpacing: '0.08em' }}>
            飛翔工程行
          </Typography>
        </Box>
        <List>
            ...
            <ListItem key={item.id} disablePadding>
              ...
              >
                <ListItemText
                  ...
                  slotProps={{
                    primary: {
                      sx: { color: '#FAF7F4', fontWeight: 500, fontSize: '1.05rem' },
                    },
                  }}
                />
              </ListItemButton>
